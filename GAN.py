...
class Gen(nn.Module):
    def __init__(self):
        ...
        self.L1 = nn.Conv2d(in_channels=256,out_channels=1024,kernel_size=3,padding=2,bias=True)
...
class Disc(nn.Module):
    def __init__(self):
        ...
        self.fc = nn.Conv2d(in_channels=1024,out_channels=1,kernel_size=3,stride=1,padding=0,bias=False)
        self.sig = nn.Sigmoid()
    def forward(self,x):
        x = self.L1(x)
        x = self.L2(x)
